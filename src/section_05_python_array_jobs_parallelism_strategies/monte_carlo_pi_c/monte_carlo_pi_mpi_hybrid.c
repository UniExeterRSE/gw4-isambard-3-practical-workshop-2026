...
static void rng_jump(Rng *r) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int b = 0; b < 64; b++) {
            if (J[i] & (UINT64_C(1) << b)) {
                ns[0] ^= r->s[0]; ns[1] ^= r->s[1];
                ns[2] ^= r->s[2]; ns[3] ^= r->s[3];
            }
            rng_next(r);
        }
    }
    r->s[0] = ns[0]; r->s[1] = ns[1]; r->s[2] = ns[2]; r->s[3] = ns[3];
}
    ...
    uint64_t base_seed, int num_threads)
{
    ...
    long long padded =
        ((chunk_size + (long long)DBL_PER_LINE - 1) / (long long)DBL_PER_LINE)
...
static int parse_args(int argc, char *argv[], Config *cfg) {
    ...
    if (e && atoi(e) > 0) default_threads = atoi(e);
    ...
    while ((opt = getopt(argc, argv, "n:d:c:s:t:h")) != -1) {
        switch (opt) {
        case 'n': cfg->n           = atoll(optarg);           break;
        case 'd': cfg->d           = atoi(optarg);            break;
        case 'c': cfg->chunk_size  = atoll(optarg);           break;
        case 's': cfg->seed        = (uint64_t)atoll(optarg); break;
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        const char *hdr =
            "variant             hits      p_hat     p_true    sigma_p"
            "     pi_hat   sigma_pi   thr ranks    time[s]";
        printf("%s\n", hdr);
        for (int i = 0; hdr[i]; i++) putchar('-');
        putchar('\n');
